...
def export_mesh_data(gfs, armature, bpy_to_gfs_node, bind_pose_matrices, errorlog, export_policies):
    ...
    # Some chunks should be separated out to sub-functions
    ...
    for bpy_mesh_object in meshes:
        ...
        if len(gfs_meshes[-1].vertices) > TooManyVerticesError.VERTEX_LIMIT:
    ...
    if len(bad_meshes):
        if too_many_vertices_policy == "IGNORE":
            ...
            errorlog.log_warning_message(f"{len(bad_meshes)} meshes need to be split into more than {TooManyVerticesError.VERTEX_LIMIT} vertices for export. This might cause issues and it is recommended to split these meshes into smaller meshes. The affected meshes are {', '.join([m.name for m in bad_meshes])}")
